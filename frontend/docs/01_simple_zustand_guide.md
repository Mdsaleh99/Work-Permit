...
That's it! Zustand is really that simple. The key is understanding that `set` updates the shared box, but returning data from actions gives you immediate access to fresh data.
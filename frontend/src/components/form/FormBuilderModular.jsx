...
const FormBuilderModular = ({
        ...
        if (isEditingMode && workPermitId) {
            const loadSubmissions = async () => {
                ...
                try {
                    ...
                    console.log(
                        "FormBuilderModular - Loaded submissions:",
                        submissions,
                    );
                    if (submissions?.data && submissions.data.length > 0) {
                        ...
                        console.log(
                            "FormBuilderModular - Latest submission:",
                            latestSubmission,
                        );
                        setSubmittedData(latestSubmission.answers);
                    } else {
                        console.log(
                            "FormBuilderModular - No submissions found",
                        );
    ...
    const handleSubmitForm = async () => {
        ...
        try {
            // Work permit number is now generated on the backend
            // No need to extract from form components

            const workPermitData = {
                title: formData.title,
                // workPermitNo will be auto-generated by backend if not provided
    ...
    if (showPrintView) {
        ...
        const formDataForPrint = (() => {
            try {
                ...
                const sections = Array.isArray(formData?.sections)
                    ? formData.sections
                    : [];
                let patched = false;
                const nextSections = sections.map((sec) => {
                    const comps = Array.isArray(sec.components)
                        ? sec.components
                        : [];
                    ...
                    return didPatchSection
                        ? { ...sec, components: nextComps }
                        : sec;
                });
        ...
        console.log(
            "FormBuilderModular Print View - Final formData:",
            finalFormData,
        );
        console.log(
            "FormBuilderModular Print View - Submitted data:",
            submittedData,
        );
        ...
        <div className="h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <FormBuilderSidebar
                    ...
                    onResetForm={
                        isEditingMode
                            ? undefined
                            : draftOperations.createNewDraft
                    }
                    onShowDraftsModal={
                        isEditingMode ? undefined : handleShowDraftsModal
                    }
                ...
                <div className="flex-1 overflow-hidden">
                    <div className="h-full overflow-y-auto">
                        <FormSection
                            ...
                            setEditingComponent={setEditingComponent}
                            onAddComponent={
                                formOperations.addComponentToSection
                            }
                            onUpdateComponent={formOperations.updateComponent}
                            onDeleteComponent={formOperations.deleteComponent}
                            onReorderComponents={
                                formOperations.reorderComponents
                            }
                ...
                <div
                    className={cn(
                        ...
                        isMobile
                            ? showComponentsPanel
                                ? "fixed right-0 top-0 h-full w-80 z-50 shadow-lg"
                                : "hidden"
                            : "w-80 flex-shrink-0",
                    )}
                >
            ...
            <DeclarationModal
                ...
                areAllDeclarationsChecked={
                    formOperations.areAllDeclarationsChecked
                }
                getDeclarationComponents={
                    formOperations.getDeclarationComponents
                }
